...
def _make_serial_conn(is_connected=True, disconnect_cb=None):
    ...
    conn._params = {
        PLUGIN_ATTR_SERIAL_PORT: '/dev/ttyUSB0',
...
class TestSendBytes(unittest.TestCase):
    ...
    def test_raises_on_serial_exception(self):
        ...
        with self.assertRaises(SDPConnectionError) as ctx:
            conn._send_bytes(b'\x04')
        self.assertIn('/dev/ttyUSB0', str(ctx.exception))
...
class TestReadBytesDisconnect(unittest.TestCase):
    ...
    def test_sets_is_connected_false_on_timeout(self):
        conn, port, cb = self._make_conn_with_instant_timeout()
        conn._is_connected = True
        conn._read_bytes(1)
        self.assertFalse(conn._is_connected)

    def test_fires_disconnect_callback_on_timeout(self):
        cb = MagicMock()
    ...
    def test_no_callback_when_already_disconnected(self):
        """If _is_connected is already False, callback should not fire again."""
        cb = MagicMock()
        conn, port, _ = self._make_conn_with_instant_timeout(disconnect_cb=cb)
        conn._is_connected = False    # already disconnected
        conn._read_bytes(1)
