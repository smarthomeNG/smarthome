...
class SDPCommands(object):
    ...
    def get_send_data(self, command: str, data: Any = None, **kwargs) -> Any:
    ...
    def update_lookup_table(self, name: str, table: str):
        """
        update lookup table
    ...
    def _lookup(self, data: str, table: str, rev: bool = False, ci: bool = True) -> Any:
        ...
        if data in lu:
            return lu[data]  # type: ignore (lu can't be list as list is only returned for mode == 'list' which is not set here)
    ...
    def _read_commands(self) -> bool:
        ...
        if hasattr(cmd_module, 'commands') and isinstance(cmd_module.commands, dict) and not SDP_standalone:  # noqa  # type: ignore
            ...
            if INDEX_GENERIC in cmds:
                ...
                if self._model:
                    known_models = (
                        set(cmd_module.commands)  # type: ignore
                        | set(getattr(cmd_module, 'lookups', {}))
                        | set(getattr(cmd_module, 'structs', {}))
                    )
                    known_models.discard(INDEX_GENERIC)
    ...
    def _parse_commands(self, commands: dict, cmds: list | None = None):
        """
