...
class SmartDevicePlugin(SmartPlugin):
    ...
    def send_command(self, command, value=None, return_result=False, **kwargs):
        ...
        # reset "last sent" data in case we receive no reply
        self.last_opcode = data_dict.get('payload', None)
        self.last_result = None
        ...
        if result:
            ...
            if return_result:
                value, custom = self._process_received_data(result, command)
                return value
            else:
    ...
    def _process_received_data(self, data, command):
        ...
        try:
            ...
            # set "last sent" data
            try:
                self.last_opcode = self._commands.get_commandlist(command)['opcode']
