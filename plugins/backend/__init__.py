...
class BackendServer(SmartPlugin):
    ...
    def my_to_bool(self, value, attr='', default=False):
        try:
            ...
            result = default
            self.logger.error("BackendServer: Invalid value '" + str(value) + "' configured for attribute " +
                              attr + " in plugin.conf, using '" + str(result) + "' instead")
        return result
    ...
    def __init__(self, sh, port=None, threads=8, ip='', updates_allowed='True', user="admin", password="", language="", developer_mode="no"):
        ...
        if self.is_int(port):
            ...
            self.port = 8383
            if port is not None:
                self.logger.error("BackendServer: Invalid value '" + str(
                    port) + "' configured for attribute 'port' in plugin.conf, using '" + str(self.port) + "' instead")

        if self.is_int(threads):
            self.threads = int(threads)
        else:
            self.threads = 8
            self.logger.error("BackendServer: Invalid value '" + str(threads) +
                              "' configured for attribute 'thread' in plugin.conf, using '" + str(self.threads) + "' instead")

        if ip == '':
            ip = self.get_local_ip_address()
            self.logger.debug(
                "BackendServer: Using local ip address '{0}'".format(ip))
        else:
            pass
        ...
        if language != '':
            if not load_translation(language):
                self.logger.warning(
                    "BackendServer: Language '{0}' not found, using standard language instead".format(language))
        self.developer_mode = self.my_to_bool(
            developer_mode, 'developer_mode', False)

        self.updates_allowed = self.my_to_bool(
            updates_allowed, 'updates_allowed', True)

        current_dir = os.path.dirname(os.path.abspath(__file__))
        self.logger.debug(
            "BackendServer running from '{}'".format(current_dir))
        ...
        self._cherrypy.tree.mount(Backend(
            self, self.updates_allowed, language, self.developer_mode), '/', config=config)
