...
def load_translation(language):
    ...
    if translation_lang == '':
        ...
        lang_filename = os.path.dirname(os.path.abspath(
            __file__)) + '/locale/' + translation_lang + '.json'
        try:
        ...
        try:
            ...
            logger.error("Backend: load_translation language='{0}': Error '{1}'".format(
                translation_lang, e))
            return False
...
def translate(txt, block=''):
    ...
    if translation_lang == '':
        ...
        if block != '':
            ...
            tr = translation_dict.get(txt, '')
        if tr == '':
            logger.warning("Backend: Language '{0}': Translation for '{1}' is missing".format(
                translation_lang, txt))
            tr = txt
