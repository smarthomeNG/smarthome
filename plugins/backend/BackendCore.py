...
class Backend:
    ...
    def find_visu_plugin(self):
        ...
        if self.visu_plugin is not None:
            ...
            if self.visu_plugin_build < '2':
                ...
                self.logger.warning("Backend: visu protocol plugin v{0} is too old to support BackendServer, please update".format(
    ...
    def system_html(self):
        ...
        if len(daytest) == 3:
            days = int(daytest[0])
            hours, minutes, seconds = [float(val)
                                       for val in str(daytest[2]).split(':')]
        else:
            days = 0
            hours, minutes, seconds = [float(val)
                                       for val in str(daytest[0]).split(':')]
        sh_uptime = self.age_to_string(days, hours, minutes, seconds)

        pyversion = "{0}.{1}.{2} {3}".format(sys.version_info[0], sys.version_info[
                                             1], sys.version_info[2], sys.version_info[3])
    ...
    def get_process_info(self, command):
        ...
        # Wait for process to terminate. The optional input argument should be
        # a string to be sent to the child process, or None, if no data should
    ...
    def getpackages(self):
        ...
        for dist in installed_packages:
            ...
            try:
                ...
                package['version_available'] = [
                    translate('Keine Antwort von PyPI')]
            packages.append(package)

        sorted_packages = sorted(
            [(i['key'], i['version_installed'], i['version_available']) for i in packages])
    ...
    def services_html(self):
        ...
        smarthome_service = self.get_process_info(
    ...
    def items_json_html(self):
        ...
        """
        items_sorted = sorted(self._sh.return_items(
    ...
    def cache_check_json_html(self):
        ...
        onlyfiles = [f for f in listdir(
            cache_path) if isfile(join(cache_path, f))]
    ...
    def item_detail_json_html(self, item_path):
        ...
        item_conf_sorted = collections.OrderedDict(
            sorted(item.conf.items(), key=lambda t: str.lower(t[0])))
        if item_conf_sorted.get('sv_widget', '') != '':
            item_conf_sorted['sv_widget'] = self.html_escape(
                item_conf_sorted['sv_widget'])
    ...
    def _build_item_tree(self, parent_items_sorted):
        ...
        for item in parent_items_sorted:
            ...
            item_data.append(
                {'path': item._path, 'name': item._name, 'tags': tags, 'nodes': nodes})
    ...
    def logics_html(self, logic=None, trigger=None, reload=None):
        ...
        self.logger.debug(
            "Backend: logics_html: trigger = '{0}', reload = '{1}'".format(trigger, reload))
        if trigger is not None:
            self.logger.debug(
                "Backend: logics_html: Trigger logic = '{0}'".format(logic))
            if self.updates_allowed:
                if logic in self._sh.return_logics():
                    self._sh.trigger(logic, by='Backend')
                else:
                    self.logger.warning(
                        "Backend: Logic '{0}' not found".format(logic))
            else:
                self.logger.warning(
        ...
        if reload is not None:
            self.logger.debug(
                "Backend: logics_html: Reload logic = '{0}'".format(logic))
            if self.updates_allowed:
                if logic in self._sh.return_logics():
                    mylogic = self._sh.return_logic(logic)
                    self.logger.info("Backend: logics_html: Reload logic='{0}', filename = '{1}'".format(
                        logic, os.path.basename(mylogic.filename)))
                ...
                self.logger.warning(
                    "Backend: Logic reloads are not allowed. (Change 'updates_allowed' in plugin.conf")
