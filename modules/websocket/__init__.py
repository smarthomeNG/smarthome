...
class Websocket(Module):
    ...
    def initialize_payload_protocols(self):
        ...
        self.ex_protocol = protocol_sync_example.Protocol(self)
        self.logger.info(f"Payload protocol '{ self.ex_protocol.protocol_name}' initialized ({'enabled' if self.ex_protocol.protocol_enabled else 'disabled'})")

        # parameters and class instance for smartVISU protocol
        # hand the websocket module instance to protocol object
        from . import protocol_smartvisu
        self.sv_protocol = protocol_smartvisu.Protocol(self)
        self.logger.info(f"Payload protocol '{ self.sv_protocol.protocol_name}' initialized ({'enabled' if self.sv_protocol.protocol_enabled else 'disabled'})")
    ...
    def set_smartvisu_support(self, protocol_enabled=False, default_acl='ro', query_definitions=False, series_updatecycle=0):
        ...
        self.sv_protocol.set_smartvisu_support(protocol_enabled, default_acl, query_definitions, series_updatecycle)
    ...
    def _ws_server_thread(self):
        ...
        if python_version == '3.6':
            self.loop.ensure_future(self.sv_protocol.update_visu())
            self.loop.ensure_future(self.sv_protocol.update_all_series())
        elif python_version == '3.7':
            self.loop.create_task(self.sv_protocol.update_visu())
            self.loop.create_task(self.sv_protocol.update_all_series())
        else:
            self.loop.create_task(self.sv_protocol.update_visu(), name='update_visu')
            self.loop.create_task(self.sv_protocol.update_all_series(), name='update_all_series')
...
#            return
        ...
        try:
            ...
            if path == self.sv_protocol.protocol_path and self.sv_protocol.protocol_enabled:
                self.logger.info(f"Starting 'smartVISU' payload protocol")
                #await self.smartVISU_protocol_v4(websocket)
                await self.sv_protocol.handle_protocol(websocket)
            elif path == self.ex_protocol.protocol_path and self.ex_protocol.protocol_enabled:
                self.logger.info(f"Starting '{self.ex_protocol.protocol_name}' payload protocol")
                await self.ex_protocol.handle_protocol(websocket)
            else:
    ...
    async def unregister(self, websocket):
        ...
        if websocket.path == self.sv_protocol.protocol_path:
            # smartVISU client
            #client_addr = self.client_address(websocket)
            #self.sv_protocol.sv_cancel_all_abos(client_addr)
            await self.sv_protocol.cleanup_connection(websocket)
        if websocket.path == self.ex_protocol.protocol_path:
            # sync_example client
            await self.ex_protocol.cleanup_connection(websocket)
    ...
    async def log_connection_event(self, action, websocket):
        ...
        if not websocket.remote_address:
            ...
            self.logger.info(f"USER {action}: {self.sv_protocol.build_client_info(websocket.remote_address)} - local port: {websocket.port} - path: {websocket.path}")
    ...
    #                         if item_acl is None:
    #                             item_acl = self.sv_protocol.sv_acl
    #                         if item_acl != 'ro':
    ...
    def set_visu_url(self, url, clientip=''):
        ...
        """
        return self.sv_protocol.set_visu_url(url, clientip)
    ...
    def get_visu_client_info(self):
        ...
        :return:
        """
        return self.sv_protocol.get_visu_client_info()
